...
def ssald(config=None):
    ...
    l_p = config.p # Noise parameter
    l_q = config.q # Noise parameter
    print("Checkpoint 0")
    ...
    for epochs in range(iterations):
        ...
        # print("No of samples discovered in this step",last_lambda.shape[0])
        ...
        #           "Samples discovered in this step": last_lambda.shape[0],
    ...
    labelled_list,p = util.labeller(query_index=final, # Labelling this correcty since this no longer has any effect on the subsequent discovery
                  label_list=label_list,
                  label = class_id,
                  l_p = l_p,
                  l_n = l_q)

    lambda_list.extend(labelled_list) ### This has a lot of mislabelling

    labelled_list,p = util.labeller(query_index=lambda_list, #Labelling it but this time correctly
                  label_list=label_list,
                  label = class_id,
                  l_p = 0,
                  l_n = 0                                  
                 )
    final_discovery = len(labelled_list)
